...
public class TweetPostFunction extends AbstractJiraFunctionProvider {
	...
	public TweetPostFunction(TweetBuilder emailBuilder, LicenseValidator licenseValidator) {
	...
	public void execute(Map transientVars, Map args, PropertySet ps) throws WorkflowException {
		...
		try {
			if (!licenseValidator.isValid()) {
				...
				TweetDefinition tweetDefiniton = buildTweet(issue, transientVars, args, ps);
	...
	@SuppressWarnings("rawtypes")
	private TweetDefinition buildTweet(MutableIssue issue, Map transientVars, Map args, PropertySet ps) {

		TweetDefinition def = new TweetDefinitionImpl(issue, getCallerUser(transientVars, args),
				(String) args.get("tweetMessageTemplate"));
